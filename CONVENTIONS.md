...
**Known cross-harness gap, project scope**: OpenCode's own docs are explicit that its external-skill auto-load only scans `~/.claude/` and `~/.agents/` — global, home-directory paths — never a project's `.claude/skills` or `.agents/skills`. `install.sh` works around this by writing a project-scoped `"skills": {"paths": [...]}` entry to the project's `opencode.json`, OpenCode's own project-scoped registration mechanism. Antigravity has a parallel but different gap: its default `.agents/skills/` directory scan has been observed going stale on a symlinked folder, so `install.sh` also writes `.agents/skills.json` (Antigravity's own documented external-registration format) as a belt-and-suspenders fix. See `install.sh`'s header comment for the mechanism-level detail.
