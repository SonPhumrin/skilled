...
`disable-model-invocation` is a Claude Code extension. It appears only on user-invoked skills, which are Claude Code entry points anyway. It is the one extension treated as free: without it there is no user-invoked mechanism at all, so the validator does not warn on it.

`argument-hint` is the other Claude Code extension in use, on four user-invoked skills (`design-workflows`, `handoff`, `handoff-to-agent`, `teach`). It is a deliberate trade rather than an oversight: the hint appears in the slash-command menu, and the cost is that those four cannot be packaged for claude.ai or the Skills API, which reject unknown keys outright. The validator warns on it so the cost stays visible. Adding a third extension means accepting that cost for every skill that carries it.

**Known cross-harness gap**: OpenCode and Antigravity both ignore frontmatter keys they do not recognize, rather than erroring on them. That makes `disable-model-invocation` safe to use everywhere (it is silently ignored, never rejected), but it also means those two harnesses have no way to honor it: every skill in this repo is model-selectable there, including the 23 meant to be typed by hand on Claude Code. There is no portable field-level fix, since no such field exists yet in the open spec. Do not work around it by inlining user-invoked content into a model-invoked skill, or the split stops meaning anything on the harnesses where it does work. The portable fix is at the harness level: install with `--model-only` and serve the user-invoked skills from `skills.json` ([HARNESS.md](HARNESS.md)).

**Cross-harness note, project scope**: current OpenCode (v1.18.30+) discovers project `.claude/skills`, `.agents/skills`, and `.opencode/{skill,skills}` natively. The installer additionally writes a project-scoped `"skills": {"paths": [...]}` entry to the project's `opencode.json` — OpenCode's own project-scoped registration mechanism — as a belt-and-suspenders addition on top of native discovery, and the thing that still matters for an `--opencode`-only symlink install or a project with native discovery disabled. Antigravity has a parallel but different gap: its default `.agents/skills/` directory scan has been observed going stale on a symlinked folder, so the installer also writes `.agents/skills.json` (Antigravity's own documented external-registration format) as a belt-and-suspenders fix. See `install.py`'s header comment for the mechanism-level detail.
...
**Context budget.** A model-invoked skill's name and description are in context in every session on every harness, so they get a far tighter budget than the spec's cap: at most 300 characters per description, and 3,600 characters of name + description across all model-invoked skills. The validator fails on the first and warns on the second; `skills.json` records the current total as `alwaysOnChars`. Adding a model-invoked skill means paying for it here, so trim before you add.

**Codex.** Codex ignores `disable-model-invocation`. Every user-invoked skill also carries `agents/openai.yaml` with `policy: allow_implicit_invocation: false`, Codex's own way of keeping a skill out of the model's catalog, and no model-invoked skill carries one. `python3 tests/validate_skills.py --emit-index` creates the file for a new user-invoked skill, and the validator enforces both directions.

**`skills.json`** is the generated catalog a harness reads (see [HARNESS.md](HARNESS.md)). Regenerate it with `--emit-index` after any change under `skills/`; the validator fails when it is stale.
