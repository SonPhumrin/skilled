...
**Known cross-harness gap**: OpenCode and Antigravity both ignore frontmatter keys they do not recognize, rather than erroring on them. That makes `disable-model-invocation` safe to use everywhere (it is silently ignored, never rejected), but it also means those two harnesses have no way to honor it: every skill in this repo is model-selectable there, including the 23 meant to be typed by hand on Claude Code. There is no portable fix, since no such field exists yet in the open spec. Do not work around it by inlining user-invoked content into a model-invoked skill, or the split stops meaning anything on the one harness where it does work.

**Cross-harness note, project scope**: current OpenCode (v1.18.30+) discovers project `.claude/skills`, `.agents/skills`, and `.opencode/{skill,skills}` natively. `install.sh` additionally writes a project-scoped `"skills": {"paths": [...]}` entry to the project's `opencode.json` — OpenCode's own project-scoped registration mechanism — as a belt-and-suspenders addition on top of native discovery, and the thing that still matters for an `--opencode`-only symlink install or a project with native discovery disabled. Antigravity has a parallel but different gap: its default `.agents/skills/` directory scan has been observed going stale on a symlinked folder, so `install.sh` also writes `.agents/skills.json` (Antigravity's own documented external-registration format) as a belt-and-suspenders fix. See `install.sh`'s header comment for the mechanism-level detail.
