...
If any of these files don't exist, **proceed silently**. Don't flag their absence; don't suggest creating them upfront. The `/domain-modeling` skill (reached via `/domain-interview` and `/architecture-review`) creates them lazily when terms or decisions actually get resolved.
