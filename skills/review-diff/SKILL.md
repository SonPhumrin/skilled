...
- An instruction to consult the design skills where the diff warrants it: call the Skill tool with "simple-first" for anything newly added, with "module-design" for a new or changed interface, with "readable-code" for naming and function shape, with "background-jobs" for a new job or queue consumer, and with "database-performance" for a new query or migration. One call per skill, only for the skills the diff actually touches.
