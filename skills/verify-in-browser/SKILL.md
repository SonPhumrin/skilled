...
- If the harness provides built-in browser tools (`browser_open`, `browser_snapshot`, `browser_click`, `browser_type`, `browser_eval`, `browser_logs`, `browser_screenshot`, possibly with an MCP prefix; see `HARNESS.md` in the skilled repo), drive its in-app browser with those and skip the `agent-browser` steps below: the user watches the run in the same pane. Steps 4 and 5 are unchanged; `browser_screenshot` saves the evidence file.
