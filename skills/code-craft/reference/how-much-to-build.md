...
**Spend novelty deliberately.** Dan McKinley's *innovation tokens*: a team has only a handful to spend before complexity outpaces its ability to operate what it built. Spend them on what makes the product different, not on swapping the boring, well-understood datastore for the new one with the interesting paper behind it. "Boring" doesn't mean bad — it means known failure modes, a deep hiring pool, and a decade of incident write-ups already published for whatever breaks at 2am. A new dependency is a bet that its payoff outweighs the token it costs; most individual bets lose even when the occasional one is worth it.

**Chesterton's Fence, before deleting someone else's.** The deletion test below is for abstractions you're about to write. For one that's already there and looks pointless, understand why it exists first — the commit that added it, the test that exercises the odd branch, the issue it links to — before clearing it away. "I don't see why this is here" is a research task, not a finding; the fence you can't explain is the one most likely guarding a bug fix nobody wrote a comment for.
