...
## First: does this need to be async at all?

A queue is infrastructure, and infrastructure defaults to no (see `how-much-to-build.md`). Do the work synchronously when:
...
If the reason is "it feels cleaner," it is not a reason. An `ARCHITECTURE.md` that says everything is synchronous is a design decision; adding the first queue changes that, so raise it rather than slipping it in.
...
**A bounded attempt count, and a budget across the whole call chain.** Unbounded retry is a queue that never drains. Worse, three retries at every layer of a chain three layers deep is not nine attempts on the system underneath, it's 4³ — sixty-four — because each layer's retry re-triggers all the retries below it. Cap attempts per request, and don't let more than one layer in a chain retry the same failure.
