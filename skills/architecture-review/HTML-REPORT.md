...
The diagrams carry the weight. Prose is sparse, plain, and uses the glossary terms (from the `/module-design` skill) without ceremony.
...
Plain English, concise, but the architectural nouns and verbs come straight from the `/module-design` skill. Concision is not an excuse to drift.
